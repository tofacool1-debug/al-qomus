...
import { analyzeSifatMusyabihatPlural, getPluralBinaKey } from "./utils/sifatMusyabihatPlural";
...
export default function App() {
  ...
  const [activeJamakTab, setActiveJamakTab] = useState<
    "fail" | "maful" | "zamanmakan" | "alat" | "sifat"
  >("fail");
    ...
    const handleNativeMessage = (event: MessageEvent) => {
      try {
        ...
        if (message && message.type === "SET_FAVORITES") {
          console.log("[WebView Bridge] Received favorites from native SQLite:", message.data);
        ...
        } else if (message && message.type === "SQLITE_STATUS") {
          console.log("[WebView Bridge] Native database is ready. Size:", message.dbSizeMB, "MB");
          setToastMsg(`Terhubung dengan Expo SQLite Luring (${message.dbSizeMB} MB)`);
        }
      } catch (err) {
  ...
  const toggleFavorite = async () => {
      ...
      (window as any).ReactNativeWebView.postMessage(
        JSON.stringify({
          ...
          payload: {
            ...
            notes: "Tersimpan via WebView Bridge di Expo SQLite"
          }
        })
      );
      setToastMsg(isCurrentlyFavorited ? "Menghapus dari SQLite lokal..." : "Menyimpan ke SQLite lokal...");
      return;
    }
    ...
    if (isCurrentlyFavorited) {
      ...
      if (favItem) {
        try {
          const res = await fetch(`/api/favorites/${favItem.id}`, { method: "DELETE" });
          if (res.ok) {
      ...
      try {
        const res = await fetch("/api/favorites", {
          ...
          body: JSON.stringify({
            ...
            translation: selectedEntry.translation,
            babNum: selectedEntry.babNum,
            notes: "Tersimpan di Cloud Database",
            custom: false
          })
        });
        if (res.ok) {
          setToastMsg("Berhasil disimpan ke daftar favorit!");
          fetchFavorites();
        }
      } catch (err) {
  ...
  const [presetGroupMode, setPresetGroupMode] = useState<
    "bab" | "bina" | "new"
  >("bab");
  const [newlyAddedEntries, setNewlyAddedEntries] = useState<DictionaryEntry[]>(
    () => {
      return [PRESET_DICTIONARY[0]];
    },
  );
    ...
    if (onlineEntries.length > 0) {
      const presetKeys = new Set(
        PRESET_DICTIONARY.map(
          (e) => `${e.root.fa}_${e.root.ain}_${e.root.lam}_${e.babNum}`,
        ),
      );

      const newItems = onlineEntries.filter(
        (e) =>
          !presetKeys.has(
            `${e.root.fa}_${e.root.ain}_${e.root.lam}_${e.babNum}`,
          ),
      );

      setNewlyAddedEntries(newItems);
    }
  }, [onlineEntries]);
        ...
        <div className="lg:col-span-4 flex flex-col space-y-4">
          ...
          >
            ...
            <div className="space-y-2.5">
              ...
              >
                {(["bab", "bina", "new"] as const).map((mode) => (
                  <button
                    key={mode}
                    type="button"
                ...
                {presetGroupMode === "new" && (
                  <div className="w-full animate-fadeIn">
                    {newlyAddedEntries.length > 0 ? (
                      ...
                      <div
                        className={`p-4 rounded-xl border text-center text-[10px] font-extrabold uppercase tracking-wider ${tc.input}`}
                      >
                        Belum ada lafadz baru ditambahkan online...
                      </div>
                    )}
                  </div>
                )}
                      ...
                      <div className="space-y-4">
                          ...
                          <table className="w-full text-left border-collapse">
                            <thead>
                              <tr className="border-b border-slate-800 bg-slate-950/60">
                                <th className="py-3 px-4 text-[10px] uppercase tracking-wider text-slate-400 font-bold font-mono">
                                  Bentuk Tasrif
                                </th>
                                <th className="py-3 px-4 text-[10px] uppercase tracking-wider text-slate-400 font-bold font-mono text-center border-l border-slate-850">
                                  Pola / Wazan Dasar
                                </th>
                                <th className="py-3 px-4 text-[10px] uppercase tracking-wider text-emerald-400 font-bold font-mono text-center border-l border-slate-850">
                                  Lafadz Terbentuk (Akar Kata)
                                </th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-850">
                              <tr>
                                ...
                                <td className="py-3 px-4 text-center text-slate-400 font-mono text-[11px] border-l border-slate-850">
                                  {structuralPlurals.sifat.isSamai ? (
                                    <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[10px] font-bold">sama'i</span>
                                ...
                                <td className="py-3 px-4 text-center font-arabic text-emerald-400 font-bold text-lg border-l border-slate-850">
                                  {renderPluralLafadz(
                                    selectedEntry.sifatMusyabihat,
                                    "text-emerald-400",
                                  )}
                              ...
                              <tr>
                                ...
                                <td className="py-3 px-4 text-center text-slate-400 font-mono text-[11px] border-l border-slate-850">
                                  {getMuannasWazanPatterns(
                                    structuralPlurals.sifat.wazan_name,
                                  )}
                                </td>
                                <td className="py-3 px-4 text-center font-arabic text-emerald-400 font-bold text-lg border-l border-slate-850">
                                  {renderPluralLafadz(
                                    (structuralPlurals.sifat as any)
                                      .mufrod_muannas,
                                    "text-emerald-400",
                                  )}
                                </td>
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="bg-slate-900/60 p-3 rounded-xl text-slate-300 border border-slate-850 text-xs leading-relaxed select-text font-serif">
                            {structuralPlurals.sifat.explanation ||
                  ...
                  <div className="space-y-4">
                    <div className="bg-emerald-950/20 border border-emerald-900/60 p-4 rounded-2xl flex gap-3 text-xs text-emerald-300">
                      ...
                      <p>
                        ...
                        analisis plurals dihitung komprehensif bagi 5 jenis Isim
                        sekaligus.
                      </p>
                    </div>
