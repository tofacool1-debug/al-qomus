...
export default function App() {
  ...
  useEffect(() => {
    if (onlineEntries.length > 0) {
      ...
      // Update newlyAddedEntries.
      if (updatedItems.length > 0) {
        // Take the latest updated item to replace the previous one
        const latestUpdate = updatedItems[updatedItems.length - 1];
        setNewlyAddedEntries([latestUpdate]);
      } else if (newItems.length > 0) {
        // Otherwise, if there's no update but there are new items, we can show them
