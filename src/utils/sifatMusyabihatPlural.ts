...
function normalizeArabicSpelling(str: string): string {
  ...
  // Replace alif with hamzah on top followed by kasra (أِ) with alif with hamzah on bottom (إِ)
  let normalized = str.replace(/أِ/g, "إِ");
  return normalized;
}
...
function substituteSifatPatternRaw(
  ...
  // Fallbacks: default literal substitution
  return pattern
    .replace(/ف/g, cleanFa)
...
export function analyzeSingleSifatMusyabihat(entry: DictionaryEntry, rawSifat: string): PluralSifatMusyabihat {
  ...
  let activeWazan = "فَعِيلٌ";
  const cleanSifat = rawSifat.replace(/[\u064b-\u065f]/g, "").trim();

  if (rawSifat.includes("أَفْعَل") || rawSifat.includes("أَحْمَر") || rawSifat.includes("أَعْمَ") || (cleanSifat.startsWith("أ") && cleanSifat.length === 4)) {
    ...
    activeWazan = "فَعِلٌ";
  } else if (rawSifat.includes("حَسَن") || rawSifat.includes("فَعَل") || cleanSifat === "حسن") {
    activeWazan = "فَعَلٌ";
  ...
  // Apply requirements 1-5 for jamak taksir & muannas
  if (activeWazan === "أَفْعَلُ") {
    ...
    qillah = "—";
    muntahal = "—";
    explanation = "Sifat Musyabihat berwazan أَفْعَلُ (muannats: فَعْلَاءُ), jamak taksirnya adalah فُعْلٌ secara qiyasi.";
    ...
    qillah = "—";
    muntahal = "—";
    explanation = "Sifat Musyabihat berwazan فَعْلَانُ (muannats: فَعْلَى), jamak taksirnya adalah فِعَالٌ secara qiyasi.";
  } else if (activeWazan === "فَعَلٌ") {
    mufrod_mudzakkar = substituteSifatPattern("فَعَلٌ", fa, ain, lam, binaKey);
    mufrod_muannas = "—";
    katsroh = substituteSifatPattern("أَفْعَالٌ", fa, ain, lam, binaKey);
    qillah = "—";
    muntahal = "—";
    explanation = "Sifat Musyabihat berwazan فَعَلٌ, jamak taksirnya adalah أَفْعَالٌ secara qiyasi/sima'i.";
    ...
    qillah = "—";
    muntahal = "—";
    explanation = "Sifat Musyabihat berwazan فَعِلٌ, jamak taksirnya adalah فَعِلُونَ atau أَفْعَالٌ.";
    ...
    qillah = "—";
    muntahal = "—";
    explanation = "Sifat Musyabihat berwazan فَعْلٌ, jamak taksirnya adalah فِعَالٌ secara qiyasi.";
  } else {
    // Default or فَعِيلٌ
    ...
    if (binaKey === "mitsal") {
      ...
      muntahal = `${substituteSifatPattern("مَفَاعِيلُ", fa, ain, lam, binaKey)} / ${substituteSifatPattern("فَعَائِلُ", fa, ain, lam, binaKey)}`;
      explanation = "Wazan فَعِيلٌ bina Mitsal memiliki jamak taksir فُعَلَاء atau فِعَال, muannats berwazan فَعِيلَة, dan shighot muntahal jumu' مَفَاعِيل / فَعَائِل.";
      ...
      mufrod_muannas = substituteSifatPattern("فَعِيلَةٌ", fa, ain, lam, binaKey);
      muntahal = `${substituteSifatPattern("مَفَاعِيلُ", fa, ain, lam, binaKey)} / ${substituteSifatPattern("فَعَائِلُ", fa, ain, lam, binaKey)}`;
      explanation = "Wazan فَعِيلٌ bina Mudhoaf memiliki jamak taksir أَفِعَّاء, muannats berwazan فَعِيلَة, dan shighot muntahal jumu' مَفَاعِيل / فَعَائِل.";
      ...
      katsroh = substituteSifatPattern("فِعَالٌ", fa, ain, lam, binaKey);
      mufrod_muannas = substituteSifatPattern("فَعِيلَةٌ", fa, ain, lam, binaKey);
      muntahal = `${substituteSifatPattern("فَعَائِلُ", fa, ain, lam, binaKey)} / ${substituteSifatPattern("مَفَاعِيلُ", fa, ain, lam, binaKey)}`;
      explanation = "Wazan فَعِيلٌ bina Ajwaf memiliki jamak taksir فِعَالٌ, muannats berwazan فَعِيلَة, dan shighot muntahal jumu' فَعَائِل / مَفَاعِيل.";
    } else {
      ...
      mufrod_muannas = substituteSifatPattern("فَعِيلَةٌ", fa, ain, lam, binaKey);
      muntahal = `${substituteSifatPattern("فَعَائِلُ", fa, ain, lam, binaKey)} / ${substituteSifatPattern("مَفَاعِيلُ", fa, ain, lam, binaKey)}`;
      explanation = "Wazan فَعِيلٌ bina Shohih memiliki jamak taksir فُعَلَاء, فِعَال, atau أَفْعِلَاء, muannats berwazan فَعِيلَة, dan shighot muntahal jumu' فَعَائِل / مَفَاعِيل.";
    }
  }
...
export function analyzeSifatMusyabihatPlural(entry: DictionaryEntry): PluralSifatMusyabihat {
  ...
  // Merge the results
  const mufrod_mudzakkars = results.map(r => r.mufrod_mudzakkar).filter(x => x && x !== "—");
  const mufrod_muannases = results.map(r => r.mufrod_muannas).filter(x => x && x !== "—");
  const katsrohs = results.map(r => r.katsroh).filter(x => x && x !== "—");
  const qillahs = results.map(r => r.qillah).filter(x => x && x !== "—");
  const muntahals = results.map(r => r.muntahal).filter(x => x && x !== "—");
