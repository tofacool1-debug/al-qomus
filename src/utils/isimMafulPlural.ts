...
function replaceRootForIsimMaful(pattern: string, fa: string, ain: string, lam: string, binaKey: string): string {
  ...
  // Ajwaf special morphing for katsroh:
  else if (binaKey === "ajwaf" && pattern === "فُعُل") {
    result = `${cleanFa}ُ${cleanAin}ُ${cleanLam}ٌ`; // e.g. قُوُلٌ / بُيُعٌ (replaces old incorrect قُلٌ)
  }
  else {
    result = pattern
      .replace(/ف/g, cleanFa)
...
export function analyzeIsimMafulPlural(entry: DictionaryEntry): PluralIsimMaful {
  ...
  switch (binaKey) {
    case "sahih":
      ...
      katsroh = formatPluralList(["فُعُل", "مَفَاعِيل"], fa, ain, lam, binaKey);
      muntahal = formatPluralList(["مَفَاعِل"], fa, ain, lam, binaKey);
      contoh = "مَكْتُوب ← كُتُب";
      ilalRule = "Tidak ada I'lal pada Bina Shohih";
      explanation = `Isim Maful untuk Bina Shohih memiliki bentuk Jamak Qillah pola 'أَفْعِلَة' (jarang digunakan), Jamak Katsroh populer pola 'فُعُل' (seperti كُتُب dari مَكْتُوب) serta 'مَفَاعِيل', dan Shighot Muntahal Jumu' berpola 'مَفَاعِل'.`;
      break;

    case "ajwaf":
      qillah = "—";
      katsroh = formatPluralList(["فُعُل"], fa, ain, lam, binaKey);
      muntahal = formatPluralList(["مَفَاعِيل"], fa, ain, lam, binaKey);
      contoh = "مَقُول ← مَقَاوِيل";
      ilalRule = "I'adah waw/ya (pengembalian huruf illat asli di posisi Ain fi'il pada bentuk jamak panjang).";
      explanation = `Isim Maful Bina Ajwaf mengurungkan pelemahan huruf tengahnya saat beralih ke jamak muntahal murni (kembali ke bentuk asalnya seperti مَقَاوِيل). Bentuk Qillah-nya ditiadakan, sedangkan Katsroh berpola 'فُعُل'.`;
      break;
    ...
    case "mitsal":
      ...
      explanation = `Isim Maful Bina Mitsal mempertahankan huruf penyakit waw di permulaan kata. Jamak Katsroh mengalir dalam pola 'مَفَاعِيل' (contoh: مَوَاعِيد) dan Shighot Muntahal Jumu' berpola 'مَفَاعِل' (contoh: مَوَاعِد).`;
      break;
    ...
    case "naqish":
      ...
      explanation = `Isim Maful Bina Naqis dijamakkan melalui Shighot Muntahal Jumu' berakhiran cacat 'مَفَاعٍ' (seperti مَرَامٍ dari مَرْمِيّ) setelah mengalami pembuangan huruf ya di akhir kalimat. Jamak Katsroh mengikuti pola 'فُعُل'.`;
      break;
    ...
    case "mudaaf":
      ...
      katsroh = formatPluralList(["فُعُل", "مَفَاعِيل"], fa, ain, lam, binaKey);
      muntahal = formatPluralList(["مَفَاعِل"], fa, ain, lam, binaKey);
      contoh = "مَحْبُوب ← مَحَابِب";
      ilalRule = "Fakk idgham (penguraian dua huruf ganda yang menyatu di lafadz mufrod).";
      explanation = `Isim Maful Bina Muda'af mengurai kembali peleburan dua huruf kembarnya (fakk idgham) pada Shighot Muntahal Jumu' 'مَفَاعِل' (contoh: مَحَابِب dari مَحْبُوب). Jamak Katsroh diwakili wazan 'فُعُل' dan 'مَفَاعِيل'.`;
      break;
    ...
    case "mahmuz":
      ...
      katsroh = formatPluralList(["فُعُل"], fa, ain, lam, binaKey);
      muntahal = formatPluralList(["مَفَاعِيل"], fa, ain, lam, binaKey);
      contoh = "مَسْؤُول ← مَسَائِل";
      ilalRule = "Ibdal hamzah (perubahan hamzah menjadi kursi ya' saat berada di sela huruf jamak).";
      explanation = `Isim Maful Bina Mahmuz mengalami perubahan penulisan kursi hamzah (ibdal hamzah) demi keselarasan makhraj huruf (contoh: مَسَائِل dari مَسْؤُول). Jamak Qillah berwazan 'أَفْعِلَة' dan Katsroh-nya berpola 'فُعُل'.`;
      break;
    ...
    case "lafif_mafruq":
      ...
      ilalRule = "Hazf 2 illat (pembuangan nahu huruf illat demi kemudahan artikulasi).";
